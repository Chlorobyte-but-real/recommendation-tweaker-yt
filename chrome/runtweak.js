...
function RequestLoadSettings() {
  chrome.runtime.sendMessage({"msgType":"load"});
}
          ...
          reader.onload = readerEvent => {
                    ...
                    if (row.length > 2 && typeof(row[0]) == typeof(true) && typeof(row[1]) == typeof("")) {
                      switch (row[1]) {
                        ...
                        case "type":
                          ...
                          break;
                        default:
                          console.log("error pokemon type " + row[1]);
                          scream = true;
                          break;
                      }
                    }
            ...
            if (!scream) {
              ...
              if (presetName && presetName.length > 0 && !presetName.startsWith("\u0000")) {
                ...
                chrome.runtime.sendMessage({"msgType":"save", "key":"CurrentRules", "data":loadedRawCurrentRules});
              }
            }
            else {
