  ...
  if ("msgType" in sentMessage) {
    switch (sentMessage.msgType) {
      case "loaded":
        ...
        break;
      case "importNoYT":
        alert("Due to a browser limitation,\nplease focus on a YouTube window.");
        break;
    }
  }
});
...
function DeletePreset(presetName) {
  ...
  if (confirm("Would you like to delete the " + presetName + " preset?")) {
    browser.runtime.sendMessage({"msgType":"save", "key":"p" + presetName, "erase":true});
...
function SavePresetBtn() {
  ...
  if (presetName.length > 0 && !presetName.startsWith("\u0000")) {
    SavePreset(presetName, GetSaveData(false));
  }
}
...
function ExportPresetBtn() {
  Download(JSON.stringify(GetSaveData(false)), "preset.json", "text/plain");
}

function ImportPresetBtn() {
  // tell background script to ask current active youtube window to import a preset for us
  browser.runtime.sendMessage({"msgType":"importPopup"});
}
